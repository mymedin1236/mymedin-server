...
// Expenses expose clinic spending — doctor-only, hidden from assistants.
router.use(protect, requireRole("doctor"));

// GET /api/expenses -> doctor's maintenance expenses (newest first)
router.get("/", async (req, res) => {
  try {
    const expenses = await Expense.find({ doctor: clinicId(req.user) }).sort({ date: -1 });
    res.json(expenses);
  } catch (err) {
    console.error(err);
